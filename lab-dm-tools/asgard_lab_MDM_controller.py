...
flat_cmd_files = [  # the flats in use
	"/usr/local/etc/flat1.txt",
	"/usr/local/etc/flat2.txt",
	"/usr/local/etc/flat3.txt",
	"/usr/local/etc/flat4.txt"
]
...
class DMTabWidget(QtWidgets.QWidget):
    ...
    def activate_flat(self):
        ...
        if self.chB_actv_flat.isChecked():
            ...
            flat_cmd = np.loadtxt(flat_cmd_files[self.dmid-1])
