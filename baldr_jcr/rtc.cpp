...
namespace
{
                               ...
                               Eigen::VectorXd &basis_vec_out)
    {
        ...
        if (basis_vec_out.size() != nAct)
            basis_vec_out.resize(nAct);
...
void rtc()
{
    ...
    while (servo_mode.load() != SERVO_STOP)
    {
        ...
        if (pause_rtc.load())
        {
            ...
            rtc_pause_cv.wait(lock, []()
                              { return !pause_rtc.load(); });
        ...
        size_t boxcar = global_boxcar.load();
        ...
        if (off)
        {
            if (servo_mode_LO.load() == SERVO_OPEN)
                ol_lo = off->lo;
            if (servo_mode_HO.load() == SERVO_OPEN)
                ol_ho = off->ho;
        }
            ...
            rtc_config.inj_signal.apply_to == "command")
        {
            ...
            if (rtc_config.inj_signal.branch == "ALL")
                c_inj_applied = a * g_inj_cache.p_all;
            else if (rtc_config.inj_signal.branch == "LO")
                c_inj_applied = a * g_inj_cache.p_lo;
            else
                c_inj_applied = a * g_inj_cache.p_ho;
        }
        ...
        if (true)
        {
            ...
            if (T >= 2)
            {
                ...
                {
                    if (x.size() != P)
                        continue; // skip anomalous frames
                    sum += x;
