...
make_controller(const std::string &type, const bdr_controller &cfg)
{
    ...
    else if (type == "LEAKY")
    {
        ...
        if (cfg.set_point.size() == n)
            c->set_setpoint(cfg.set_point);
        return c;
    }
    else if (type == "KALMAN")
    {
...
std::string send_cam_cmd(const std::string &command)
{
    ...
    zmq::message_t reply;
    zmq::recv_result_t result = cam_zmq_socket.recv(reply, zmq::recv_flags::none);
...
void set_openloop_offset_LO(const Eigen::Ref<const Eigen::VectorXd> &lo)
{
    ...
    {
        std::cerr << "[OL] set_lo: not initialized.\n";
        return;
    }
    auto cur = std::atomic_load_explicit(&g_ol_offsets, std::memory_order_acquire);
...
void set_openloop_offset_HO(const Eigen::Ref<const Eigen::VectorXd> &ho)
{
    if (!ol_init_if_needed())
    {
        std::cerr << "[OL] set_ho: not initialized.\n";
        return;
    }
    auto cur = std::atomic_load_explicit(&g_ol_offsets, std::memory_order_acquire);
...
nlohmann::json poll_telem_vector(std::string name)
{
    static const std::unordered_map<std::string,
                                    boost::circular_buffer<Eigen::VectorXd> bdr_telem::*>
        telemetry_map = {
            {"img", &bdr_telem::img},
...
nlohmann::json poll_telem_scalar(std::string name)
{
    ...
    static const std::unordered_map<std::string,
                                    boost::circular_buffer<double> bdr_telem::*>
        scalar_map = {
    ...
    for (Eigen::Index r = 0; r < M.rows(); ++r)
    {
        if constexpr (std::is_integral_v<S>)
        {
            ...
            for (Eigen::Index c = 0; c < M.cols(); ++c)
                row[c] = static_cast<long long>(M(r, c));
            rows.push_back(row);
        }
        else
        {
            ...
            for (Eigen::Index c = 0; c < M.cols(); ++c)
                row[c] = static_cast<double>(M(r, c));
            rows.push_back(row);
        }
    }
    return rows;
...
static bool json_to_eigen_vector(const nlohmann::json &j, EigenVec &out)
{
    ...
    try
    {
        for (size_t i = 0; i < n; ++i)
            out(static_cast<Eigen::Index>(i)) = static_cast<S>(j.at(i).get<double>());
        return true;
    }
    catch (...)
    {
...
static Controller *pick_ctrl(const bdr_rtc_config *cfg, const char *which)
{
    ...
    if (lo)
        return cfg->ctrl_LO ? cfg->ctrl_LO.get() : nullptr;
    if (ho)
        return cfg->ctrl_HO ? cfg->ctrl_HO.get() : nullptr;
    return nullptr;
}
...
static FieldHandle make_scalar_field_getter(T bdr_rtc_config::*ptr, const char *type_name)
{
    return FieldHandle{
        type_name,
        [ptr]() -> nlohmann::json
        { std::lock_guard<std::mutex> lk(rtc_mutex); return rtc_config.*ptr; },
        nullptr};
}
...
static FieldHandle make_nested_scalar_getter(Sub bdr_rtc_config::*sub, T Sub::*mem, const char *type_name)
{
    return FieldHandle{
        ...
        { std::lock_guard<std::mutex> lk(rtc_mutex); return (rtc_config.*sub).*mem; },
        nullptr};
}
template <typename T>
...
static FieldHandle make_nested_eigen_vector_getter(Sub bdr_rtc_config::*sub, EigenVec Sub::*mem, const char *tn = "vector")
{
    return FieldHandle{
        tn,
        [sub, mem]() -> nlohmann::json
        { std::lock_guard<std::mutex> lk(rtc_mutex); return eigen_vector_to_json((rtc_config.*sub).*mem); },
        nullptr};
}
...
static FieldHandle make_nested_eigen_matrix_getter(Sub bdr_rtc_config::*sub, EigenMat Sub::*mem, const char *tn = "matrix")
{
    return FieldHandle{
        tn,
        [sub, mem]() -> nlohmann::json
        { std::lock_guard<std::mutex> lk(rtc_mutex); return eigen_matrix_to_json((rtc_config.*sub).*mem); },
        nullptr};
}
template <typename EigenVec>
static FieldHandle make_eigen_vector_field_getter(EigenVec bdr_rtc_config::*mem, const char *tn = "vector")
{
    return FieldHandle{
        tn,
        [mem]() -> nlohmann::json
        { std::lock_guard<std::mutex> lk(rtc_mutex); return eigen_vector_to_json(rtc_config.*mem); },
        nullptr};
}
template <typename EigenMat>
static FieldHandle make_eigen_matrix_field_getter(EigenMat bdr_rtc_config::*mem, const char *tn = "matrix")
{
    return FieldHandle{
        tn,
        [mem]() -> nlohmann::json
        { std::lock_guard<std::mutex> lk(rtc_mutex); return eigen_matrix_to_json(rtc_config.*mem); },
        nullptr};
}
...
nlohmann::json ctrl_list(nlohmann::json args)
{
    ...
    if (!c)
        return json{{"ok", false}, {"error", "controller not initialized"}, {"which", which}};
    ...
    for (const auto &[name, desc] : c->list_parameters())
    {
        ...
        const char *ty =
            std::holds_alternative<Eigen::VectorXd>(p) ? "vector<double>" : std::holds_alternative<Eigen::MatrixXd>(p) ? "matrix<double>"
                                                                                                                       : "unknown";
...
nlohmann::json ctrl_get(nlohmann::json args)
{
    ...
    try
    {
        ...
        {
            return json{{"ok", true}, {"which", which}, {"name", name}, {"type", "vector<double>"}, {"value", eigen_vector_to_json(std::get<Eigen::VectorXd>(p))}};
        }
        if (std::holds_alternative<Eigen::MatrixXd>(p))
        {
            return json{{"ok", true}, {"which", which}, {"name", name}, {"type", "matrix<double>"}, {"value", eigen_matrix_to_json(std::get<Eigen::MatrixXd>(p))}};
        }
    ...
    catch (const std::exception &e)
    {
        ...
        for (const auto &kv : c->list_parameters())
...
nlohmann::json ctrl_set(nlohmann::json args)
{
...

    {
        ...
        else
        {
            ...
            {
                return json{{"ok", false}, {"error", "value is neither vector nor matrix"}, {"which", which}, {"name", name}};
            }
        }
    } // lock released here
...
void I0_update_lucky(double percentile = 90.0, int min_samples = 10)
{
    ...
    for (size_t i = 0; i < K; ++i)
    {
        if (metrics[i] >= thresh)
        {
            ...
            if (fdm.size() == P_dm)
                acc_dm += fdm; // skip mismatched silently
...
nlohmann::json update_ctrl_param(nlohmann::json args)
{
    ...
    try
    {
        ...
        if (parameter_type == "setpoint")
            parameter_type = "set_point";

        // choose controller
        std::unique_ptr<Controller> *which = nullptr;
        if (controller_str == "lo")
            which = &rtc_config.ctrl_LO;
        else if (controller_str == "ho")
            which = &rtc_config.ctrl_HO;
        else
            return json{{"error", "Invalid controller: use \"LO\" or \"HO\""}};

        if (!(*which))
            return json{{"error", "Controller is not initialized"}};
...
void build_interaction_matrix(double poke_amp = 0.05, int num_iterations = 10,
    double sleep_seconds = 0.01, const std::string &signal_space = "dm",
...
//
namespace
{ // anonymous namespace fpr probe_methods saving
    ...
    inline std::vector<std::string> parse_field_list(const nlohmann::json &jfield)
    {
        if (jfield.is_string())
        {
            ...
            if (iequals(s, "all"))
                return list_all_numeric_telem_fields();
            return {std::move(s)};
        }
        if (jfield.is_array())
        {
...
namespace dm
{
                                       ...
                                       std::string &warn_msg)
    {
        ...
        if (mode_ax != 2)
            warn_msg = "Mode axis not NAXIS3; auto-detected.";
        return true;
    }
    ...
    inline bool load_from_fits_file(const std::string &fits_path)
    {
        ...
        for (int i = 2; i <= hdunum; ++i)
        { // start at 2 (skip primary)
            ...
            if (en.size() < 7 || en.rfind("BASIS:", 0) != 0)
                continue; // not starting with BASIS:
            ...
            if (!warn.empty())
                std::cerr << "[DMBasis] NOTE: " << warn << " (EXTNAME=" << en << ")\n";
            ...
            if (common_nx == 0 && ny > 0)
            { // first successful ext with 3D or 2D
                common_nx = nx;
                common_ny = ny;
            }
            else
    ...
    try
    {
        ...
        else if (args.is_object())
        {
            ...
            if (args["amplitude"].is_number())
                ...
                return json{{"ok", false}, {"error", "amplitude must be numeric"}};
            if (args.contains("accumulate"))
                accumulate = args["accumulate"].get<bool>();
        }
        else if (args.is_string())
        {
            std::istringstream iss(args.get<std::string>());
        ...
        if (mode.size() != dm_len)
            return json{{"ok", false}, {"error", "basis mode length != DM length"}, {"basis_len", (long)mode.size()}, {"dm_len", dm_len}};
        ...
        {
            if (accumulate)
                set_openloop_offset_HO(cur->ho + v);
            else
                set_openloop_offset_HO(v);
        }
        else
        {
            if (accumulate)
                set_openloop_offset_LO(cur->lo + v);
            else
                set_openloop_offset_LO(v);
        }
...
static inline int telem_field_kind(const std::string &f)
{
    ...
    if (f == "img" || f == "img_dm" || f == "signal" || f == "e_LO" || f == "u_LO" ||
        f == "e_HO" || f == "u_HO" || f == "c_LO" || f == "c_HO")
        return 0;
    if (f == "timestamp" || f == "rmse_est" || f == "snr")
        return 1;
    if (f == "LO_servo_mode" || f == "HO_servo_mode")
        return 2;
    return -1;
}
...
capture_telem_list(const std::string &field, size_t N, long long dead_ms, bool change_capacity = true)
{
    ...
    std::vector<std::vector<double>> out;
    {
        ...
        case 0: // vector fields
            if (field == "img")
                dump_vecbuf(rtc_config.telem.img);
            else if (field == "img_dm")
                dump_vecbuf(rtc_config.telem.img_dm);
            else if (field == "signal")
                dump_vecbuf(rtc_config.telem.signal);
            else if (field == "e_LO")
                dump_vecbuf(rtc_config.telem.e_LO);
            else if (field == "u_LO")
                dump_vecbuf(rtc_config.telem.u_LO);
            else if (field == "e_HO")
                dump_vecbuf(rtc_config.telem.e_HO);
            else if (field == "u_HO")
                dump_vecbuf(rtc_config.telem.u_HO);
            else if (field == "c_LO")
                dump_vecbuf(rtc_config.telem.c_LO);
            else if (field == "c_HO")
                dump_vecbuf(rtc_config.telem.c_HO);
            else if (field == "c_inj")
                dump_vecbuf(rtc_config.telem.c_inj); // <-- add this
            break;
        case 1: // scalar<double>
            if (field == "timestamp")
                dump_dblbuf(rtc_config.telem.timestamp);
            else if (field == "rmse_est")
                dump_dblbuf(rtc_config.telem.rmse_est);
            else if (field == "snr")
                dump_dblbuf(rtc_config.telem.snr);
            break;
        case 2: // scalar<int>
            if (field == "LO_servo_mode")
                dump_intbuf(rtc_config.telem.LO_servo_mode);
            else if (field == "HO_servo_mode")
                dump_intbuf(rtc_config.telem.HO_servo_mode);
            break;
        }
    }
...
capture_telem_multi(const std::vector<std::string> &fields, size_t N, long long dead_ms, bool change_capacity = true)
{
    ...
    // Snapshot all requested fields under one mutex
    {
        ...
        for (const auto &field : fields)
        {
            ...
            case 0: // vector fields
                if (field == "img")
                    dump_vecbuf(rtc_config.telem.img, dst);
                else if (field == "img_dm")
                    dump_vecbuf(rtc_config.telem.img_dm, dst);
                else if (field == "signal")
                    dump_vecbuf(rtc_config.telem.signal, dst);
                else if (field == "e_LO")
                    dump_vecbuf(rtc_config.telem.e_LO, dst);
                else if (field == "u_LO")
                    dump_vecbuf(rtc_config.telem.u_LO, dst);
                else if (field == "e_HO")
                    dump_vecbuf(rtc_config.telem.e_HO, dst);
                else if (field == "u_HO")
                    dump_vecbuf(rtc_config.telem.u_HO, dst);
                else if (field == "c_LO")
                    dump_vecbuf(rtc_config.telem.c_LO, dst);
                else if (field == "c_HO")
                    dump_vecbuf(rtc_config.telem.c_HO, dst);
                else if (field == "c_inj")
                    dump_vecbuf(rtc_config.telem.c_inj, dst); // new
                break;
            case 1: // scalar<double>
                if (field == "timestamp")
                    dump_dblbuf(rtc_config.telem.timestamp, dst);
                else if (field == "rmse_est")
                    dump_dblbuf(rtc_config.telem.rmse_est, dst);
                else if (field == "snr")
                    dump_dblbuf(rtc_config.telem.snr, dst);
                break;
            case 2: // scalar<int>
                if (field == "LO_servo_mode")
                    dump_intbuf(rtc_config.telem.LO_servo_mode, dst);
                else if (field == "HO_servo_mode")
                    dump_intbuf(rtc_config.telem.HO_servo_mode, dst);
                break;
            }
        }
    }
...
nlohmann::json probe_interaction_data(nlohmann::json args)
{
    ...
    try
    {
        if (args.is_array())
        {
            ...
            if (args.size() >= 6)
                dead_ms = args.at(5).get<long long>();

            const auto &arr = args.at(0);
            if (!arr.is_array())
                return json{{"ok", false}, {"error", "aberrations must be array"}};
            for (const auto &t : arr)
            {
                if (!t.is_array() || t.size() != 2)
                    return json{{"ok", false}, {"error", "each aberration must be [idx,amp]"}};
        ...
        else if (args.is_object())
        {
            ...
            { // have to deal with case of "all" input so FIELD headers isnt too long and just writes "all"
                const auto &farg = *it;
                if (farg.is_string())
                {
                    std::string s = farg.get<std::string>();
            ...
            if (args.contains("deadtime_ms"))
                dead_ms = args.at("deadtime_ms").get<long long>();
    ...
    if (args.is_object() && args.contains("outfile"))
    {
        try
        {
            outfile = args.at("outfile").get<std::string>();
        }
        catch (...)
        {
        }
...
nlohmann::json tt_inj(nlohmann::json args)
{
    ...
    try
    {
        if (args.is_array())
        {
            ...
            {
                return json{{"ok", false}, {"error", R"(usage: ["chirp|prbs|sine", amplitude?, "setpoint|command"?])"}};
            }
            method = args.at(0).get<std::string>();
            if (args.size() >= 2)
                amplitude = args.at(1).get<double>();
            if (args.size() >= 3)
                apply_to = args.at(2).get<std::string>();
        }
        else if (args.is_object())
        {
            if (args.contains("method"))
                method = args.at("method").get<std::string>();
            if (args.contains("amplitude"))
                amplitude = args.at("amplitude").get<double>();
            if (args.contains("apply_to"))
                apply_to = args.at("apply_to").get<std::string>();
        }
        else
        {
...
static int save_probe_result_fits(const nlohmann::json &jr, const std::string &filepath)
{
    ...
    if (NSAMP <= 0)
        throw std::runtime_error("[probe] first probe has zero samples");

    const long VLEN = static_cast<long>(s0[0].size()); // vector length per sample
    if (VLEN <= 0)
        throw std::runtime_error("[probe] first sample has zero length");
...
static nlohmann::json build_probe_args_for_method(const std::string &method)
{
    ...
    if (method == "I2A")
    {
        ...
        for (int iii : {50, 53, 86, 89})
        { // the four inner corners (4 rows in)
...
static nlohmann::json run_probe_method(nlohmann::json args)
{
    ...
    try
    {
        if (args.is_array())
        {
            ...
            method = args.at(0).get<std::string>();
            if (args.size() == 2)
                out_path = args.at(1).get<std::string>();
        }
        else if (args.is_object())
        {
            if (!args.contains("method"))
            ...
            if (args.contains("path"))
                out_path = args.at("path").get<std::string>();
        }
        else
        {
...
COMMANDER_REGISTER(m)
{
    ...
    m.def("pause_baldr_rtc", []()
          {
        pauseRTC();
        return std::string("RTC paused."); }, "Pause the RTC loop.\n\n");

    m.def("resume_baldr_rtc", []()
          {
        resumeRTC();
        return std::string("RTC resumed."); }, "Resume the RTC loop.\n\n");

    // build_interaction_matrix [0.05, 10, 0.1, "dm", "/home/asg/Music/IM_test.fits"]
    m.def("build_interaction_matrix", [](json args)
          {
            ...
            return json{{"status", "interaction matrix construction complete"}}; }, "Builds interaction matrix: build_interaction_matrix [poke_amp, num_iter, sleep_s, signal_space, output_filename]", "args"_arg);
    ...
    m.def("ol_set_mode", [](json args)
          { return ol_set_mode(args); }, "ol_set_mode — set/add an open-loop offset in HO or LO using a basis mode; usage: [\"HO|LO\",\"<basis>\",<index>,<amplitude>,(optional)<accumulate_bool>] (accumulate defaults false).");
