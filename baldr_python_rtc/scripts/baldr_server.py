...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Baldr RTC Python server (per-beam instance)."
    )
    ap.add_argument(
        "--beam",
        type=int,
        required=True,
        choices=[1, 2, 3, 4],
        help="Beam/telescope index (1-4).",
    )
    ap.add_argument(
        "--phasemask",
        default="H3",
        choices=[f"H{ii}" for ii in [1, 2, 3, 4, 5]]
        + [f"J{ii}" for ii in [1, 2, 3, 4, 5]],
        help="ZWFS phasemask used. Index 1 is smallest, 5 is largest. H is H-band in Baldr, J is J-band in Baldr",
    )
    ap.add_argument(
        "--socket",
        default=None,
        help="Override commander REP endpoint, e.g. tcp://127.0.0.1:3001",
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=1000,
        help="Telemetry ring capacity (samples).",
    )
    ap.add_argument(
        "--flush-hz", type=float, default=1, help="Telemetry flush rate (Hz)."
    )
    ap.add_argument(
        "--chunk-seconds",
        type=float,
        default=0.1,
        help="Telemetry chunk duration (seconds).",
    )
    ap.add_argument(
        "--debug",
        action="store_true",
        help="Enable more verbose printing and/or plots for debugging",
    )
    args = ap.parse_args()
    ...
    telem_dir = (
        args.telem_dir
        or f"/home/asg/ben_feb2026_bld_telem/{tstamp_rough}/beam{args.beam}"
    )
